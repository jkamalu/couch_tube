...
joinButton.addEventListener('click', function(event) {
	...
	joinInput.value = ''
	socket.emit('JOIN FROM CLIENT', {roomName})
})
...
socket.on('JOIN FROM SERVER', function(data) {
	room = data; console.log(room)
	statusLabel.className = 'label label-success'
	statusLabel.innerHTML = 'CONNECTED to ' + data.room_name.toUpperCase() + ' as ' + data.join_as.toUpperCase()
})
...
function onPlayerStateChange(event) {
	...
	if (playerState == 1 || playerState == 2 || playerState == 3) {
		socket.emit('PLAYER STATE CHANGE FROM CLIENT', {playerState: playerState, currentTime: player.getCurrentTime()})
	} else {}
}
